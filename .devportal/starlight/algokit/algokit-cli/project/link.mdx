...
The `algokit project link` command is a powerful feature designed to streamline the integration between `frontend` and `contract` typed projects within the AlgoKit ecosystem. This command facilitates the automatic path resolution and invocation of [`algokit generate client`](/algokit/algokit-cli/docs/algokit/algokit-cli/generate#typed-clients) on `contract` projects available in the workspace, making it easier to integrate smart contracts with frontend applications.
...
To learn more about the `algokit project link` command, please refer to [link]() in the AlgoKit CLI reference documentation.
